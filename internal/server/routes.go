...
// wrap applies recovery + CORS middleware to a handler.
func wrap(h http.HandlerFunc) http.HandlerFunc {
	return handler.Recover(handler.Cors(h))
}

// RegisterRoutes wires all API and static file routes to the default mux.
func RegisterRoutes(staticFS http.Handler, homeFile string) {
	// LAN API
	...
	http.HandleFunc("/", staticHandler(staticFS, homeFile))
}
